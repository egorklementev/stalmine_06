...
public class TECarousel extends TileEntity  implements ITickable {
    ...
    public TECarousel() {
        timer = StalmineConfig.carouselTimer;
        artTimer = StalmineConfig.carouselArtTimer;
        soundStarted = false;
    }
    ...
    public void update() {
        ...
        if (entList.isEmpty() && timer != StalmineConfig.carouselTimer) {
            resetTimer();
        }
        for (EntityLivingBase ent : entList) {
            if (ent != null && !(ent instanceof EntityPlayer && ((EntityPlayer) ent).isCreative())) {
                ...
                if (hit) {
                    ((BlockCarousel)world.getBlockState(getPos()).getBlock()).showBlast(Minecraft.getMinecraft().world, getPos());
                }
                ...
                if (!entList.isEmpty() && !hasSoundStarted()) {
                    ...
                    if (world.isRemote) {
                        Minecraft.getMinecraft().world.playSound(
                                ...
                                getPos().getY(),
                                getPos().getZ(),
                                StalmineSounds.pool.get("carousel_blast"),
        ...
        if (!world.isRemote) {
            if (artTimer <= 0) {
                artTimer = StalmineConfig.carouselArtTimer + world.rand.nextInt(StalmineConfig.carouselArtTimer / 2);
                ...
                if (chance > StalmineConfig.art_legendary) {
                    ArrayList<ItemArtifact> arts = new ArrayList<ItemArtifact>() {
                        {
                            add(StalmineItems.itemGoldfish);
