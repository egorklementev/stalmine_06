...
public class TESteam extends TileEntity  implements ITickable {
    ...
    public TESteam() {
        timer = 0L;
        artTimer = StalmineConfig.steamArtTimer;
    }
    ...
    public void resetTimer() {
        timer = StalmineConfig.steamTimer;
        markDirty();
    ...
    public void update() {
        ...
        if (!world.isRemote) {
            if (artTimer <= 0) {
                artTimer = StalmineConfig.steamArtTimer + world.rand.nextInt(StalmineConfig.steamArtTimer / 2);
        ...
        if (timer >= 0) {
            if (timer == StalmineConfig.steamTimer) {
                Minecraft.getMinecraft().world.playSound(
                        pos.getX(),
                        pos.getY(),
                        pos.getZ(),
                        StalmineSounds.pool.get("steam_blast"),
            ...
            timer--;
            ((BlockSteam)getWorld().getBlockState(getPos()).getBlock()).showBlast(getWorld(), getPos());
        }
        markDirty();
        if (!getWorld().isRemote) {
            getWorld().notifyBlockUpdate(
